...
class NGCF(object):
    def __init__(self, data_config, pretrain_data, generator):
        # argument settings
        train_dataset = tf.data.Dataset.from_generator(generator.sample_iterator, (tf.int32, tf.int32, tf.int32), 
                        (tf.TensorShape([None]), tf.TensorShape([None]), tf.TensorShape([None]))).repeat()
        iterator = train_dataset.make_one_shot_iterator()
        self.users, self.pos_items, self.neg_items = iterator.get_next()
        ...
        *********************************************************
        ...
        self.node_dropout = tf.constant(eval(args.node_dropout))
        self.mess_dropout = tf.constant(eval(args.mess_dropout))
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    model = NGCF(data_config=config, pretrain_data=pretrain_data, generator=data_generator)
    ...
    if args.pretrain == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                ret = test_iter(sess, model, data_generator, drop_flag=True)
    ...
    *********************************************************
    ...
    '''if args.report == 1:
        ...
        f.write(
            'embed_size=%d, lr=%.4f, layer_size=%s, regs=%s, loss_type=%s, adj_type=%s\n'
            % (args.embed_size, args.lr, args.layer_size, args.regs, args.loss_type, args.adj_type))
